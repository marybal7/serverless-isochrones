...
def handler(event, context):
    ...
    table_nodes_highway = f"highways/Moscow/nodes_{timestamp}"
    table_edges_highway = f"highways/Moscow/edges_{timestamp}"
    ...
    def create_tables():
        def create(session):
            ...
            session.execute_scheme(f"""
                CREATE TABLE `{table_nodes_highway}` (
                    id Uint64,
                    lat Float,
                    lon Float,
                    PRIMARY KEY (id)
            ...
            """)
            session.execute_scheme(f"""
                CREATE TABLE `{table_edges_highway}` (
                    edge_id Uint64,
    ...
    def get_existing_tables():
        full_path = os.path.join(os.getenv("YDB_DATABASE"), "highways/Moscow")
        try:
    ...
    def get_oldest_table(prefix):
        ...
        return f"highways/Moscow/{sorted_tables[0]}"
    ...
    try:
        ...
        for el in data["elements"]:
            if el["type"] == "way" and "highway" in el.get("tags", {}):
                ...
                if maxspeed_raw:
                    try:
                        avg_speed_kmh = float(maxspeed_raw.split()[0])
                    except ValueError:
                        avg_speed_kmh = maxspeed_aliases.get(maxspeed_raw.strip(),
                                                             highway_defaults.get(highway_type, 60))
                else:
                    avg_speed_kmh = highway_defaults.get(highway_type, 60)
        ...
        logger.info(f"Всего автомобильных рёбер: {len(ways_highway)}")
        ...
        for edge in ways_highway:
            highway_node_ids.update([edge["node_from"], edge["node_to"]])
        logger.info(f"Подготовлено {len(highway_node_ids)} highway узлов для вставки")

        insert_node_table(table_nodes_highway, highway_node_ids)
        insert_edge_table(table_edges_highway, ways_highway)

    except Exception as e:
        logger.error(f"ОШИБКА ВО ВРЕМЯ ИМПОРТА: {e}")
        logger.warning("Удаляем только что созданные таблицы...")
        drop_table_if_exists(table_nodes_highway)
        drop_table_if_exists(table_edges_highway)
        raise
    ...
    return {
        "statusCode": 200,
        "body": json.dumps({
            "highway_nodes": len(highway_node_ids),
            "highway_edges": len(ways_highway),
            "tables": {
                "nodes": table_nodes_highway,
                "edges": table_edges_highway
            },
            "deleted_old_tables": {
